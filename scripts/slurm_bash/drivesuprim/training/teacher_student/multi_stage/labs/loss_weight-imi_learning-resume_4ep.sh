...
do
  submit_job \
      ...
      --partition $partition \
      --account av_research \
      -c ". /lustre/fsw/portfolios/av/users/shiyil/yaowenh/pre-navsim_v2.sh; bash $bash_file $change_loss_weight $use_imi_learning_in_refinement $epoch"
...
: '
usage:
bash scripts/slurm_bash/ssl/training/teacher_student/multi_stage/labs/loss_weight-imi_learning-resume_4ep.sh \
    scripts/ssl/training/teacher_student/rot_30-trans_0-va_0-p_0.5/multi_stage/labs/loss_weight-imi_learning-resume.sh \
    interactive \
