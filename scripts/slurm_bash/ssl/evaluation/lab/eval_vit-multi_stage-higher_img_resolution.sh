...
submit_job \
    ...
    --tasks_per_node 8 \
    --nodes 1 \
    -n "eval-navsim_ssl-$dir_name-${epoch}epoch-higher_img_resolution" \
    ...
    --partition $partition \
    --account av_research \
    -c ". /lustre/fsw/portfolios/av/users/shiyil/yaowenh/pre-navsim_v2.sh; bash scripts/ssl/evaluation/lab/eval_vit-multi_stage-higher_img_resolution.sh $epoch $dir $num_refinement_stage $stage_layers $topks"


: '
usage:
bash scripts/slurm_bash/ssl/evaluation/lab/eval_vit-multi_stage-higher_img_resolution.sh \
