...
submit_job \
    ...
    --partition $partition \
    --account av_research \
    -c ". /lustre/fsw/portfolios/av/users/shiyil/yaowenh/pre-navsim_v2.sh; bash scripts/ssl/evaluation/eval_vit-multi_stage-navhard_two_stage.sh $epoch $dir $num_refinement_stage $stage_layers $topks $use_first_stage_traj_in_infer"
...
bash scripts/slurm_bash/ssl/evaluation/eval_vit-multi_stage-navhard_two_stage.sh \
    ...
    interactive \
    -num_refinement_stage=1 -stage_layers=3 -topks=256 -use_first_stage_traj_in_infer=false
'
