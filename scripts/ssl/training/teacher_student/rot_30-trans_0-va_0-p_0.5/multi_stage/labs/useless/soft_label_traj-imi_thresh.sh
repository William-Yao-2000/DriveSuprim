...
soft_label_traj=$1
soft_label_imi_diff_thresh=$2

dir=training/ssl/teacher_student/rot_$rot-trans_$trans-va_$va-p_$probability/multi_stage/labs/stage_layers_$stage_layers-topks_$topks

if [ "$soft_label_traj" = "final" ]; then
    dir="$dir-use_final_soft_label_traj"
fi

dir="$dir-imi_thresh_$soft_label_imi_diff_thresh"
...
command_string="python $NAVSIM_DEVKIT_ROOT/navsim/planning/script/run_training_ssl.py \
    ...
    agent.config.soft_label_traj=$soft_label_traj \
    agent.config.soft_label_imi_diff_thresh=$soft_label_imi_diff_thresh \
