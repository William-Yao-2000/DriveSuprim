...
if [ "$inference_model" = "teacher" ]; then
    experiment_name="${dir}/test-${padded_epoch}ep-one_stage-higher_img_resolution-not_use_imi_in_refinement-navhard_two_stage"
else
    experiment_name="${dir}/test-${padded_epoch}ep-${inference_model}-one_stage-higher_img_resolution-not_use_imi_in_refinement-navhard_two_stage"
fi
...
python ${NAVSIM_DEVKIT_ROOT}/navsim/planning/script/run_pdm_score_gpu_ssl.py \
    ...
    agent.config.lab.use_imi_learning_in_refinement=false \
