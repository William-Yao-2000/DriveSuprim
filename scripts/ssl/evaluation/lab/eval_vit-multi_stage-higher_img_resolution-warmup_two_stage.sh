...
if [ "$inference_model" = "teacher" ]; then
    experiment_name="${dir}/test-${padded_epoch}ep-one_stage-higher_img_resolution-warmup_two_stage"
else
    experiment_name="${dir}/test-${padded_epoch}ep-${inference_model}-one_stage-higher_img_resolution-warmup_two_stage"
fi
...
: '
usage:
bash scripts/ssl/evaluation/lab/eval_vit-multi_stage-higher_img_resolution-warmup_two_stage.sh \
  5 training/ssl/teacher_student/rot_30-trans_0-va_0-p_0.5/multi_stage/labs/stage_layers_3-topks_256-higher_img_resolution \
  1 3 256 true
'