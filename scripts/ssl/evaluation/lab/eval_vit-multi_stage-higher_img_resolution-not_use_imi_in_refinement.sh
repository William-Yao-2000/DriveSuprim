...
if [ "$inference_model" = "teacher" ]; then
    experiment_name="${dir}/test-${padded_epoch}ep-one_stage-higher_img_resolution-not_use_imi_in_refinement"
else
    experiment_name="${dir}/test-${padded_epoch}ep-${inference_model}-one_stage-higher_img_resolution-not_use_imi_in_refinement"
fi
