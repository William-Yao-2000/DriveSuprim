...
if [ "$inference_model" = "teacher" ]; then
    experiment_name="${dir}/test-${padded_epoch}ep-one_stage-offset_refinement"
else
    experiment_name="${dir}/test-${padded_epoch}ep-${inference_model}-one_stage-offset_refinement"
fi
...
python ${NAVSIM_DEVKIT_ROOT}/navsim/planning/script/run_pdm_score_one_stage_gpu_ssl.py \
    ...
    agent.config.refinement.use_offset_refinement_v2=true \
...
: '
usage:
bash scripts/ssl/evaluation/lab/eval_vit-multi_stage-offset_refinement.sh \
    6 training/ssl/teacher_student/rot_30-trans_0-va_0-p_0.5/multi_stage/labs/stage_layers_3-topks_256-offset_refinement \
    1 3 256
'