...
from navsim.planning.metric_caching.metric_cache_processor_aug_train import MetricCacheProcessorAugTrain
from navsim.planning.scenario_builder.navsim_scenario import NavSimScenario

logger = logging.getLogger(__name__)


def cache_scenarios_aug_train(
    ...
    def cache_scenarios_internal_aug_train(args: List[Dict[str, Union[Path, DictConfig]]]) -> List[CacheResult]:
        def cache_single_scenario_aug_train(
            scene_dict: Dict[str, Any], processor: MetricCacheProcessorAugTrain
        ...
        def cache_single_synthetic_scenario_aug_train(
            scene_path: Path, processor: MetricCacheProcessorAugTrain
        ) -> Optional[CacheMetadataEntry]:
        ...
        processor = MetricCacheProcessorAugTrain(
        ...
        for idx, scene_dict in enumerate(scene_loader.scene_frames_dicts.values()):
            ...
            file_cache_metadata = cache_single_scenario_aug_train(scene_dict, processor)
            gc.collect()
        ...
        for idx, (scene_path, _) in enumerate(scene_loader.synthetic_scenes.values()):
            ...
            file_cache_metadata = cache_single_synthetic_scenario_aug_train(scene_path, processor)
            gc.collect()
    ...
    result = cache_scenarios_internal_aug_train(args)
...
def cache_data_aug_train(cfg: DictConfig, worker: WorkerPool) -> None:
    """
    ...
    cache_results = worker_map(worker, cache_scenarios_aug_train, data_points)
