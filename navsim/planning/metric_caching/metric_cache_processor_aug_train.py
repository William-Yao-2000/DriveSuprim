...
class MetricCacheProcessorAugTrain:
    ...
    def _get_planner_inputs(self, scenario: AbstractScenario) -> Tuple[PlannerInput, PlannerInitialization]:
        ...
        """
        initial frame, information about ego_states and observations
        ego: (global) x,y,heading; (ego coord) v, accer; timestamp
        ...
        history.ego_state_buffer[0] = ego_state_augmentation(history.ego_state_buffer[0], _rotation, _vel, _acc)
    ...
    def compute_metric_cache(self, scenario: NavSimScenario) -> MetricCache:
        ...
        return MetricCache(
            ...
            ego_state=ego_state_augmentation(scenario.initial_ego_state, _rotation, _vel, _acc),
...
def ego_state_augmentation(ego_state: EgoState, rotation: float = 0.0, vel: float = 0.0, acc: float = 0.0) -> EgoState:
    """
    Augment ego state with rotation and velocity/acceleration perturbations
    Args:
        ego_state: Original ego state
        rotation: Rotation angle in radians, positive means turning left
        vel: Velocity perturbation coefficient 
        acc: Acceleration perturbation coefficient
    Returns:
        Augmented ego state
    """
    es_ret = copy.deepcopy(ego_state)
    
    # Apply rotation
    ret_heading = es_ret._car_footprint._center.heading + rotation
    if ret_heading < -np.pi:
        ret_heading += (2*np.pi)
    elif ret_heading > np.pi:
        ret_heading -= (2*np.pi)
    es_ret._car_footprint._center.heading = ret_heading
    
    # Apply velocity/acceleration perturbations
    es_ret._dynamic_car_state._rear_axle_velocity_2d._x *= (1.0 + vel)
    es_ret._dynamic_car_state._rear_axle_velocity_2d._y *= (1.0 + vel)
    es_ret._dynamic_car_state._rear_axle_acceleration_2d._x *= (1.0 + acc)
    es_ret._dynamic_car_state._rear_axle_acceleration_2d._y *= (1.0 + acc)
