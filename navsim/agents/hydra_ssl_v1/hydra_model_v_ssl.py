...
from navsim.agents.hydra_ssl_v1.hydra_backbone_pe_v_ssl import HydraBackbonePE
...
class TrajOffsetHead(nn.Module):
    ...
    def forward(self, bev_feat_fg, refinement_dict) -> Dict[str, torch.Tensor]:
        ...
        for i in range(self.num_stage):
            ...
            scores = (
                    0.5 * last_layer_result['noc'].log() +
                    0.5 * last_layer_result['da'].log() +
                    8.0 * (5 * last_layer_result['ttc'] + 2 * last_layer_result['comfort'] + 5 * last_layer_result['progress']).log()
            )
