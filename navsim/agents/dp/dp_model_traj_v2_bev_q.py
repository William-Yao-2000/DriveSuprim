...
from navsim.agents.dp.dp_model_traj_v2_bev import DPHead_v2
from navsim.agents.hydra_plus.hydra_backbone_bev_q import HydraBackboneBEVQ


class DPModel_traj_v2_bev_q(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor],
        ...
        for k, v in features.items():
            if 'camera' in k and isinstance(v, list):
                features[k] = v[-1]
        status_feature: torch.Tensor = features["status_feature"][0]

        img_tokens, bev_tokens, up_bev = self._backbone(features["camera_feature"], features["camera_feature_back"],
                                                        features["camera_feature_left"], features["camera_feature_right"])
