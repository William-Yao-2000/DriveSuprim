...
from navsim.agents.hydra_plus.hydra_model import HydraTrajHead
from navsim.agents.transfuser.transfuser_model import AgentHead


class HydraModelBEV(nn.Module):
    ...
    def extract_bev(self, features):
        ...
        return keyval, up_bev, status_encoding

    def forward(self, features: Dict[str, torch.Tensor],
                interpolated_traj=None) -> Dict[str, torch.Tensor]:
        keyval, up_bev, status_encoding = self.extract_bev(features)
    ...
    def evaluate_dp_proposals(self, features, dp_proposals):
        keyval, up_bev, status_encoding = self.extract_bev(features)
        output: Dict[str, torch.Tensor] = {}
        trajectory = self._trajectory_head.eval_dp_proposals(keyval, status_encoding, dp_proposals)
        output.update(trajectory)
        return output